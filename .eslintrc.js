...
module.exports = {
  ...
  rules: {
    ...
    // 允许出现空函数
    'no-empty-function': IS_ENV_PRODUCTION ? ['error'] : ['warn'],
    // 禁止使用空解构模式
    ...
    'no-undef': ['error'],
    // 禁止出现未使用过的变量
    'no-unused-vars': IS_ENV_PRODUCTION ? ['error'] : ['warn'],
    // 禁止在变量定义之前使用它们
    ...
    // 禁止出现多行空行
    'no-multiple-empty-lines': IS_ENV_PRODUCTION ? ['error'] : ['warn'],
    // 禁用 Object 的构造函数
    'no-new-object': ['error'],
    // 禁用 tab
    'no-tabs': ['error'],
    // 禁用行尾空白
    'no-trailing-spaces': IS_ENV_PRODUCTION
    ...
    // 强制在大括号中使用一致的空格
    'object-curly-spacing': IS_ENV_PRODUCTION ? ['error', 'always'] : ['warn', 'always'],
    // 强制操作符使用一致的换行符
